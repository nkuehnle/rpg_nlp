...
def plot_highly_variable_words(
    result: pd.DataFrame,
    save_path: Path,
    n_top: int = 5000,
):
    ...
    """
    subset = result.highly_variable
    var = result.variances
    var_norm = result.variances_norm
    means = result.means
    ...
    for idx, d in enumerate(data):
        ...
        for label, color, mask in zip(
            ["highly variable words", "other words"],
            ["black", "grey"],
            [subset, ~subset],
        ):
            means_, vars_ = means[mask], d[mask]
            plt.scatter(means_, vars_, label=label, c=color, s=1)

        if idx == 0:
            plt.legend()
        plt.xlabel(("$log_{10}$ " if False else "") + "mean occurences of words")
        data_type = "variances"
        plt.ylabel(
            ("$log_{10}$ " if False else "")
            + "{} of words".format(data_type)
        ...
        y_min = np.min(var)
        y_min = (
            0.95 * y_min if y_min > 0 else 1e-2
        )  # Can't plot negative with logcale in matplotlib
        y_max = np.max(var) * 1.05

        plt.xscale("log")
        x_min = np.min(means)
        x_min = (
            0.95 * x_min if x_min > 0 else 1e-2
        )  # Can't plot negative with logcale in matplotlib
        x_max = np.max(means) * 1.05
